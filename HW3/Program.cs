...
namespace TaskScheduler
{
    ...
    public class TaskScheduler
    {
        ...
        public void AddTask(IScheduledTask task)
        {
            ...
            _tasks.Add(task);
            Console.WriteLine($"Task {task.Name} was added.");
        }

        public void RemoveTask(string taskName)
        {
            // TODO: Remove task from the scheduler
            var remove = _tasks.RemoveAll(task => task.Name == taskName);
            if (remove > 0)
            {
                Console.WriteLine($"The {taskName} was removed.");
            }
            else {
                Console.WriteLine($"The task was not found to be removed.");
            }
        }
        ...
        public async Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            Console.WriteLine("Schedule is running");

            while (!cancellationToken.IsCancellationRequested)
            {

                var sortedList = _tasks.OrderByDescending(task => task.Priority).ToList();
                foreach (var task in sortedList)
                {
                    if (DateTime.Now - task.LastRun >= task.Interval)
                    {
                        Console.WriteLine($"[{DateTime.Now:HH:mm:ss} Excuting {task.Name}]");
                        await task.ExecuteAsync();
                    }
                }
                await Task.Delay(500, cancellationToken);
            }
            Console.WriteLine("The schedule stopped.");
        }
